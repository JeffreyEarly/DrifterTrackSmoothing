...
% w = @(z) sigma_gps^2*ones(size(z));

% order 1e-8 for Jerk condition

% [mx,my,Cmx,Cmy,X,V,A] = forcing_fit_cauchy(t,x,y,dx,dy,T_decorrelation,M,S,1e3,0,w);
%[mx,my,Cmx,Cmy,X,V,A] = forcing_fit_cauchy(t,x,y,dx,dy,T_decorrelation,M,S,1e-8,0,w);
[mx,my,Cmx,Cmy,X,V,A,J,dx2,dy2] = forcing_fit_cauchy(t,x,y,dx,dy,T_decorrelation,M,S,u_rms,0,w);
...
epsilon_v = [Diff1*x-u1;Diff1*y-v1];
xi = (-0.20:0.001:0.20)';

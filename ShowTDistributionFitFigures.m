...
sigma_a = 0.7e-5; nu_a = 5.5;
acceleration_tpdf = @(z) gamma((nu_a+1)/2)./(sqrt(pi*nu_a)*sigma_a*gamma(nu_a/2)*(1+(z.*z)/(nu_a*sigma_a*sigma_a)).^((nu_a+1)/2));

% r=linspace(0,100,1000);
% pdf1d = tdistpdf(r/sigma_a,nu)/sigma_a;
% r(end+1)=1000; pdf1d(end+1) = 0.0;
% velocity_tdistpdf = @(z) interp1(r,pdf1d,z);
...
for iDrifter = 1:Ndrifters
    ...
    Ba = bspline(t_a,t_knot,K);
    X = squeeze(B(:,:,1));
    error_ax = [error_ax; Diff2*(x-X*m_x)];
    error_ay = [error_ay; Diff2*(y-X*m_y)];
...
%     ax = A*m_x;
%     ay = A*m_y;
%     a_total = sqrt( ax.*ax + ay.*ay );
%     badNuts = a_total > 5e-5;
    ...
    if (iDrifter == choiceDrifter)
        ...
        fx_fit_big = ax_fit_big - drifters.f0*v_fit_big;

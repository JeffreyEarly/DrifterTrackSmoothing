...
%
%------------------------------------------------------------------------------
function [m_x,m_y,Cm_x,Cm_y,X,V,A,J] = drifter_fit_forced(t,x,y,dx,dy,T_decorrelation,M,S,v0,lat0, weight_function)
...
Sigma = WeightingFunction(t,T_decorrelation);
Wx = Sigma*diag(1./(dx.^2));
Wy = Sigma*diag(1./(dy.^2));
...
dbstop if warning
[m_x,m_y,Cm_x,Cm_y] = ComputeSolution( X, Xq, Vq, XV, VV, F_force, F_constraint, D, W, Wx, Wy, gamma, f0, M, NC, x, y, h );
...
%         dy1(find(abs(dy1)<1e-12)) = 1e-12;
      ...
      ds1 = max(abs(dx1),abs(dy1));
        ...
        Wx = Sigma*diag(1./(dx2.^2));
        Wy = Sigma*diag(1./(dy2.^2));
        Ws = Sigma*diag(1./(ds2.^2));
        
%          dbstop if warning
        [m_x,m_y,Cm_x,Cm_y] = ComputeSolution( X, Xq, Vq, XV, VV, F_force, F_constraint, D, W, Ws, Ws, gamma, f0, M, NC, x, y, h );
...
function [m_x,m_y,Cm_x,Cm_y] = ComputeSolution( X, Xq, Vq, XV, D2, F_force, F_constraint, D, W, Wx, Wy, gamma, f0, M, NC, x, y, h )
    % A (and D) matrix:
    % Rows are the N observations
    % Columns are the M splines
    % F is NCxM

...
\end{equation}
where $x^{(n)}$ is the $n$-th derivative of $x$. Eventually $\Gamma$ will increase above 1? This is incorrect, the factor $\sqrt{n!}$ should be replaced with $\sqrt{A_n}$ where $A_n=\{2,6,20,70,252,..\}$. These values can be found from summing the square of the columns of the differentiation matrices.
...
\begin{equation}
w(\epsilon_i) = \sigma^2 \frac{\nu}{\nu+1} \left( \frac{\nu \sigma^2 + \epsilon_i^2}{\nu \sigma^2} \right)
\end{equation}
which is just,
\begin{equation}
w(\epsilon_i) = \sigma^2 \frac{\nu  + \frac{\epsilon_i^2}{\sigma^2}}{\nu+1}
\end{equation}
So now its easy to see how the reweighting works.
...
Using the technique of simply extending the time interval, i.e., dropping points in between, we get some excellent separation that looks really damn good. We know the covariance of, for example, $\epsilon^{\textrm{dv}}$, a change in velocity, so we know exactly what noise to compare to for significant change.

A key point in the algorithm at the moment is that we add a new knot point only when the difference exceeds the noise threshold. But, this makes the problem asymmetric because we're only going fowards in time. Once we cross a significant threshold, we need to walk backwards in time until maximum statistical separation is found. God I hope that makes sense. In other words, maximize the z-score between the two vectors. Fantastic!

This is very straight, dirt simple algorithm for creating knot points for $K=1$ ($S=0$), a piecewise constant function. All you do is add a new point to a set, and then recompute the mean. If all the points in that set are within 2 or 3 $\sigma$ of the mean, then you're good. If they're not, then you start a new set. That works beautifully.

So when do we decide what to do with velocity? Any of the previous sets tell us that we saw a significant change in position. And thus, we definitely observed a velocity. So we compute those velocities. We then do the same thing as before with position. 

At level 0, we look for significant changes in position. At level 1, we look for significant changes in velocity. Etc.

If we do find significant changes in position, then it means we have a nonzero velocity and $S\geq1$. But, we don't yet know where to put the knot points for $S=1$, because we don't which velocity are significantly different from each other. If all the velocities are indistinguishable, but nonzero, then we'd be fitting to a linear trend.

So the second part to the algorithm has to be to sweep through the velocities, and find those that are not distinguishable from each other. You probably have to start with the two neighbors that are most alike, combine them, then look around again. The reason is that every time you combine a group, you may change the way its neighbors get grouped.

Possible title: Smoothing noisy GPS data with optimal B-spline knot placement

The only issue now is figuring out how this scales to higher derivatives.

When we group neighboring velocities, we are, apparently doing a time-weighted average. That's the only way it makes sense following the definition of the derivative. However, it's not clear how we add the variances in the same way.

\begin{align}
v_1 + \epsilon^v_1 \equiv& \frac{x_2-x_1}{t_2-t_1} + \frac{\epsilon_2 - \epsilon_1}{t_2-t_1}\\
v_2 + \epsilon^v_2 \equiv& \frac{x_3-x_2}{t_3-t_2} + \frac{\epsilon_3 - \epsilon_2}{t_3-t_2}
\end{align}
And then we define the new time-weighted average,
\begin{equation}
\bar{v}_{12} \equiv \frac{\Delta t_1 (v_1 + \epsilon^v_1) + \Delta t_2 (v_2 + \epsilon^v_2)}{\Delta t_1 + \Delta t_2}
\end{equation}
and we can try to compute the covariance.
\begin{align}
\textrm{cov}(\bar{v}_{12}, \bar{v}_{12}) =& \frac{ \textrm{cov}(\Delta t_1 (v_1 + \epsilon^v_1) + \Delta t_2 (v_2 + \epsilon^v_2), \Delta t_1 (v_1 + \epsilon^v_1) + \Delta t_2 (v_2 + \epsilon^v_2)) }{ (\Delta t_1 + \Delta t_2)^2 } \\
=& \frac{1}{(\Delta t_{12})^2} \textrm{cov}(\Delta t_1 (v_1 + \epsilon^v_1), \Delta t_1 (v_1 + \epsilon^v_1) + \Delta t_2 (v_2 + \epsilon^v_2)) \\ \nonumber &+ \frac{1}{(\Delta t_{12})^2} \textrm{cov}(\Delta t_2 (v_2 + \epsilon^v_2), \Delta t_1 (v_1 + \epsilon^v_1) + \Delta t_2 (v_2 + \epsilon^v_2)) \\
(\Delta t_{12})^2 \textrm{cov}(\bar{v}_{12}, \bar{v}_{12}) =& \textrm{cov}(\Delta t_1 (v_1 + \epsilon^v_1), \Delta t_1 (v_1 + \epsilon^v_1) ) +  \textrm{cov}(\Delta t_1 (v_1 + \epsilon^v_1), \Delta t_2 (v_2 + \epsilon^v_2)) \\ \nonumber &+ \textrm{cov}(\Delta t_2 (v_2 + \epsilon^v_2), \Delta t_1 (v_1 + \epsilon^v_1) ) + \textrm{cov}(\Delta t_2 (v_2 + \epsilon^v_2),  \Delta t_2 (v_2 + \epsilon^v_2)) \\
=& (\Delta t_1)^2 \textrm{cov}( \epsilon^v_1, \epsilon^v_1) + 2 \Delta t_1 \Delta t_2 \textrm{cov}(\epsilon^v_1,\epsilon^v_2) +  (\Delta t_2)^2 \textrm{cov}( \epsilon^v_2, \epsilon^v_2) 
\end{align}
This is exactly what you'd hope for: we essentially undo the time scalings, add everything up, then scale with the new time. Specifically, in the covariance matrix we multiply the columns and then the rows by the $\Delta t$ between the points they represent, then sum the columns and rows over which we're averaging, then divide by the new $\Delta t^2$.

So what is,
\begin{align}
\textrm{cov} (\bar{v}_{12}, \bar{v}_{34}) =& \textrm{cov} \left( \frac{\Delta t_1 \epsilon^v_1 + \Delta t_2 \epsilon^v_2}{\Delta t_1 + \Delta t_2}, \frac{\Delta t_3 \epsilon^v_3 + \Delta t_4 \epsilon^v_4}{\Delta t_3 + \Delta t_4} \right) \\
=& \frac{\Delta t_2 \Delta t_3}{ \left( \Delta t_1 + \Delta t_2 \right) \left( \Delta t_3 + \Delta t_4\right)} \textrm{cov} ( \epsilon^v_2, \epsilon^v_3)
\end{align}
Same pattern as before. Multiply by the old $\Delta t$ (once for the column, once for the row), then divide by the new $\Delta t$ (once for the column, once for the row).

Note that the time-weighted acceleration is just the difference between the first and last velocity, divided by time. All the intermediate information is essentially garbaged, just as in the velocity average. This does not seem like the best use of statistical power because the end point velocities still have relatively high variance.

The best I can come up with at the moment is to minimize the error of the acceleration for a particular grouping by choosing an optimal set of velocities to difference. Or we go back to the earlier idea of using the results from the velocity difference significance. It's not clear where you'd place knot points. And, you'd still not have detected non-accelerations that are statistically significant. In other words, no change in velocity could be a statistically significant observation. So knot points would be between the velocity difference knot points. You'd have one where the acceleration changes value to the other observation acceleration. But you might also have a second one, in between which the acceleration drops to zero. That's probably a doable algorithm, but I don't really like it.

One key point here is that the information gleaned from the velocity sorting algorithm is helpful. Each point where there's a significant change in velocity, means that there's potentially a significant change in acceleration. So there's no sense in looking else where for such an inflection point. This point could take the left and right velocity vector to be different lengths. However, a 'non-change' in velocity would almost certainly be centered in time to maximize the statistical power (reduce the error).

\section{Rethinking}

I am back to believe there is nothing different about looking for changes in position than looking for changes in velocity.

You start by grouping things with the lowest z-score together until all groups have a minimum z-score from each other. So now these are statistically significant changes in position.

Now you want to start to find statistically significant changes in velocity. Imagine a particle at rest for 10 seconds which then moves with a slow velocity for 10 more seconds. The noise level is such that you get several points for each statistically significant change in position during the nonzero velocity phase.

No no, start again.

Group by significant changes in position.

To check for significant changes in velocity, create the velocity between first points of two neighboring groups, and the last points of those same groups. If there's a significant difference, then it means you found a change in velocity. Actually, you use the mean value returned by the least squares fit, but you use the $\Delta t$ determined by the first and last points as suggested. 

first point/last point test is done during the first sweep to determine whether or not we tear down the wall (knot point) between two groupings. We tear down the walls as necessary walls that remain indicate we still have significant changes in velocity (or position).

I think there's something very right about this.

Essentially you use the mean value of the grouping (for both position and time) to compute the velocity, then you do a $z$-test on all the velocities. You combine the worst scoring, but *only* if the first point/last point test shows no significant change.

You use the standard error from the best fit to assess whether things are statistically distinguishable. This is assessing constant changes in position from others.

You can write this algorithm as a local assessment of each grouping/knot point. So if the change in two velocities looks the same as the changes in the two neighboring velocities, eliminate the point between them.

\section{It gets better}

The results of the maximum likelihood fit actually give us both the mean, and the error for the fit. The error for the coefficients on the fit is computed $E\indices{^i_j} =  \left(X\indices{_k^i} \left(\Sigma^{-1}\right)\indices{^k_m}  X\indices{^m_j}\right)^{-1}$. Recall $X\indices{^i_j}$ is the spline basis where the first index references the $N$ observations at times $t_i$ and the second index references the $M$ knot points. We currently spit this quantity out at the end of the spline fitting algorithm.

If you then conjugate by the basis, you can determine the errors in position, (or velocity or whatever), including the covariance structure. Specifically, the \emph{standard error in position} is ${SE_X}=X E X^T$, while the \emph{standard error in velocity} is ${SE_V}=V E V^T$. Note that this provides the covariance structure as well. At the highest order of the spline (where it is piece

When you group \emph{positions}, you place knot points between the position data.

When you group \emph{velocities}, you place knot points between the velocity data, which has the effect of placing knot points on the position data (for evenly spaced data).

Logically then, when you group \emph{accelerations}, you place knot points between the acceleration data, which has the effect of placing knot points on the velocity data, which has the effect of placing knot points between the position data (for evenly spaced data).

At each level you possible double the knot points. After position groups are finalized, you consider changes in velocity to the left and to the right of the position knot point. Same thing goes with acceleration, you consider acceleration knot points to the left and to the right of the velocity knot points.

Thinking about the simplest cases of acceleration. Let's assume you have zero velocity, followed by some constant velocity, followed by some zero velocity and that the constant velocity takes place between two data points. You then want three knot points placed.

\subsection{No-velocity detection test}

Compute the left-hand velocity, and the right-hand velocity, then compare. If significant, then you still need a knot point. You put it on the right point of the left group if the lefthand velocity is absolutely smaller. You put it on the left point of the right group if the righthand velocity is absolutely smaller.

\section{What I'm actually doing}

\subsection{Positions}
I'm trying to use the z-test for positions. This is roughly $z = \frac{\bar{x} - \mu_0}{s}$ where $\bar{x}$ is the sample mean, $\mu_0$ is the hypothesis and $s^2=\sigma^2/n$. The quantity $s$ is referred to as the standard error of the mean. The z-test differs from the t-test in that we know $\sigma^2$, instead of using the sample standard deviation. When comparing two data sets, you'd use $z = \frac{\bar{x}_1 - \bar{x}_0}{\sqrt{ s_1^2 + s_2^2}}$.

Let's say we have two groups of data, $g^1$ and $g^2$. When I write $g^1.x$, this refers to the set of data points, $x_i$, in the first group. Likewise, $g^1.n$ refers to the number of of data points in that set.

I'm defining things in the following way for each group $g^i$,
\begin{align}
g^i.\mu =& \frac{1}{g^i.n} \sum_{k}^{g^i.n} g^i.x_k \\
g^i.s =&  \frac{1}{g^i.n} \sum_{k}^{g^i.n} g^i.\sigma_k^2
\end{align}
The test statistic between groups $i$ and $i+1$ is,
\begin{equation}
z = \frac{g^{i+1}.\mu - g^i.\mu}{\sqrt{g^{i+1}.s^2 + g^i.s^2} }
\end{equation}
You can think of the quantity $\sqrt{g^{i+1}.s^2 + g^i.s^2}$ as being the pooled standard error, z simply the difference in the means in units of that value.

The final test is whether $z>3$, my cutoff for things looking sufficiently different.

\subsection{Velocities}
I'm trying to use the same test statistic for velocities, but now there are some additional complications.

First, the standard error appears be defined as,
\begin{equation}
s^2 = \frac{ \frac{1}{n-2} \sum_k^n (x_k - \hat{x}(t_k))^2}{\sum_k^n (t_k - \bar{t})^2}.
\end{equation}
Structurally this looks like the sample variance, weighted towards the end points. I'm not sure this is what we want, because we think we already know the variance, right?

The other thing, is that the the two samples might covary in some way. So we need to include the effect of covariance. The pooled standard error when then look like,
\begin{equation}
\sqrt{g^{i+1}.s^2 + g^i.s^2 - 2 \textrm{cov}(g^{i+1},g^i)}
\end{equation}

\subsection{Algorithm termination}

We need to look to see if the value (of position, velocity, acceleration, etc.) is significantly different than zero. If it is, then we continue, if it's not, then we have a way for the algorithm to terminate.

However, I think we also may want to apply this earlier on.

Example: let's say you find all the positions and group them all together. Some of these groups will be different than zero, some of them will not. It seems like one should `force' the ones that are not different from zero, to actually be zero. Then sweep through the z-score matching again to see if things further reduce.

Now you go to the the velocity stage where you divided the knot points. Do the usual z-score matching. Is there is a region indistinguishable from zero? Then force it to zero by constraint. This has no bearing on the standard error---that is determined from the knot points alone, if we discard the additional constraint changes to the error which only reduces error. So THEN we loop through again, see if we can combine regions (after all, this could have moved two means closer together, although it won't have changed the standard errors). In regions where the velocity was set to zero, you then need to ask if the position should be set to zero! Maybe an additional constraint will be necessary.

BUT, just because the mean position in some region is indistinguishable from zero, doesn't mean it's velocity won't be... right?

Why force to zero? This allows the algorithm to be adaptive to what the data is doing---expressing ignorance where appropriate. Different regions will have different orders of fit.

Doing this with Lagrange constraints makes the most sense---we already have the formula's and error analysis from Teanby---and conceptually I think this is correct as well. It's easy to see how we constrain the highest derivative. Pick a single point in the group as the spot to apply the constraint. Teanby writes `adding constraints always reduces the errors in the parameters' --- does this seem right? Maybe we don't want to include the additional error correction---after all, we're not saying we know additional information when we set the derivative to zero, we're saying that we can't be certain that it's different from zero.

Notes on June 8th, 2016:
Generalized the algorithm to be recursive, which may have fixed a bug or two, because the quality of fits are much better.

However, adding constraints doesn't quite seem to do what I thought it should. The problem is that I'm not propagating constraint information up the recursion. Intuitively, we need to be deciding whether or not two groups should be merged, OR whether one or the other should be set to zero. Somehow this idea needs to be integrated into the problem on the ground level. Basic hypothesis testing this is, said Yoda.

Maybe compute a z-score for merging and a z-score for zeroing. The lowest is what we do.

Need to NOT include the z-score for zeroing from regions we've already zeroed. THEN we need to remove the constraints of merged groups. The way to do this is to 'flag' groups as being zeroed... include this in the struct.

\subsection{Standard error}

Here are some notes looking at the standard error.

At $S=0$, group together more points gives a variance of $\sigma^2/N$ where $N$ is the number of points grouped. So that makes sense.

At $S=1$, adding one additional point decreased by $2\sigma^2/f(N)$ where $f(N) = \{ 1, 4, 10, 20, 35\}$.

At $S=2$, adding one additional point decreased by $18.74 \sigma^2/f(N)$ where $f(N) = \{ 1.0,8.24,34.61,103.65, 252.69\}$, approximately. These ones aren't round.

So for S=0, this makes perfect sense. Even S=1 makes some sense, because each additional point is a multiplier on $\Delta t$ and the square root appears to do a touch better, which also makes sense. But this is WAY over inflated  for S=2. This is dropping the error much too quickly.

I'm finding a threshold of 3.7 is optimal when $\sigma=25$ in my default test case, but 2.7 when $\sigma=50$. That's tough.

\subsection{Error analysis}

How do we determine if we have a best fit? Simply computing the rms error of the position isn't sufficient, because we also want a way to characterize how good the fit is at higher derivatives as well. So, we want to take into account the rms error of the velocity and acceleration, for example. A fit that performs better at those derivatives is useful.

One useful quantity is the error assuming the value is zero. So, that is to say $\alpha^{(1)} = \sqrt{\frac{1}{Q}\sum (x^{\textrm{true}} )^2}$ is the error you'd get by assuming the position is always zero. The quantity $\alpha^{(n)}$ indicates the rms value of the $n$-th derivative of the true position. The rms error that I actually get with my fit is $\beta^{(1)} = \sqrt{\frac{1}{Q}\sum (x^{\textrm{true}}- x^{\textrm{fit}} )^2}$. This is a useful quantity because $\beta/\alpha$ gives me 0 if I've done a perfect fit, $1$ if I've done as good as assuming the null hypothesis and greater than 1, if I'm doing worse than assuming zero.

Define an error metric,
\begin{equation}
Q_\textrm{error} = \frac{1}{n} \sum \left( \frac{\beta^{(n)}}{\alpha^{(n)}} -1 \right)
\end{equation}
that sums to zero if you postulate the particle didn't move. If your fit does \emph{worse} than simply postulating no motion, you will get a positive value, if you do better, you will get a negative value. You are penalized for fitting worse than the null hypothesis. This means that you're better off expressing your ignorance about higher derivatives (and therefore setting them to zero) rather than making bad, ill-informed guesses.

\section{Relationship with finite difference matrix}

A second order finite difference matrix has the same coefficients as the values of the second derivative of a 2nd order spline, 1, -2, 1.

Remember my computation of the variances deduced from a finite difference matrix? Those are just $D \cdot \Sigma \cdot D^{\textrm{T}}$


\section{Mixed order splines}

Here we create a basis of splines that are not of uniform order $K$. The idea is to have some splines with lower order zero derivatives.
...
\end{cases}
\end{equation}
Normally higher order B-splines are defined by recursion,
\begin{equation}
X^K_m(t) \equiv \frac{t - t_m}{t_{m+K-1} - t_m} X^{K-1}_m(t) + \frac{t_{m+K}-t}{t_{m+K} - t_{m+1}} X^{K-1}_{m+1}(t).
\end{equation}

We will keep this same recursion, but modify the initial value as follows. Given a collection of groups, each with a max order, $k_i$,
\begin{equation}
X^{K}_m(t) \equiv \begin{cases}
1      & \text{if $ \tau_m \leq t < \tau_{m+1}$ and $K =  K_{\textrm{max}} - k_i + 1$}\\
0     & \text{otherwise}.
\end{cases}
\end{equation}
The recursion is the used when $K >  K_{\textrm{max}} - k_i + 1$

I think this will result in the same number of nonzero splines. And, there's no reason why you shouldn't just repeat the initial and final knot points to $K_{\textrm{max}}-1$. 

\section{Tension weighting}

The expected PDF will not a $\chi^2$ distribution.

Assume that the physical process has true value $x(t_i)$ at time $t_i$ and is stationary, but we observe value $x_i$ given by $x_i = x(t_i) + \epsilon_i$ where $\epsilon_i$ given by uncorrelated Gaussian noise with variance $\sigma_x$. Then the total power of the measured signal will be
\begin{equation}
...
\end{equation}
where $u(t_i)$ and $u_i$ are the true and measured velocities of the process, respectively. This statement remains true for all higher order derivatives.

This can all be rewritten as,
\begin{equation}
 \frac{1}{N}\sum \left( x_i^2 - x(t_i)^2 \right) = \frac{1}{N} \sum \epsilon_i^2
\end{equation}
and,
\begin{equation}
 \frac{1}{N} \sum \left( u_i^2 -  u(t_i)^2\right) = \frac{1}{N} \sum \left(\mathcal{D} \epsilon_i\right)^2
\end{equation}
which essentially just tell us that in fact, expected PDF should be treated as a $\chi^2$ distribution. Duh.

Okay, so the first condition is really just that,
\begin{equation}
E \left(  \frac{1}{N}\sum \left( x_i^2 - x(t_i)^2 \right) \right) = \sigma_x^2
\end{equation}

We could treat the higher derivatives in the same manner, BUT they're actually telling us something more directly about the tension. In particular,
\begin{equation}
 \frac{1}{N} \sum u(t_i)^2   = \frac{1}{N} \sum \left( u_i^2 - \left(\mathcal{D} \epsilon_i\right)^2 \right)
\end{equation}
gives us the tension parameter for velocity, exactly.


\section{More on tension}

Here are a few things that I've learned.

\begin{enumerate}
\item First note that the expected spectrum of the noise is white, but its amplitude is a function of sampling frequency. So $S(f) = \sigma^2 \Delta$ where $\sigma^2$ is the total variance of the signal and $\Delta$ is the sampling frequency. This makes sense on two levels 1) the total variance of the signal has to sum to $\sigma^2$, so going to higher frequencies is going to `spread' the signal out more and 2) the more we sample a given frequency, the more we expect its power to drop because we're increasing our degrees of freedom.
\item Be careful interpreting the reduction of tension when going to more sparsely sampled processes. Specifically, because I've lumped the $\chi^2$ normalization and the acceleration normalization into one parameter, it could be that I'm simply getting the $\chi^2$ normalization incorrect. In fact, this appears to be largely the case. Renormalizing the output with the resulting $\chi^2$ value of the optimal fit gives nearly the correct (known) acceleration back out, although only within a factor of 2.
\item The big question comes down to trying to figure out how to normalize the $\chi^2$ like term---in other words, how many degrees of freedom we have.
\item Without question, the answer to the above depends on the signal itself.
\end{enumerate}
...
x(t) = A \cos( 2\pi f t)
\end{equation}
contaminated by Gaussian noise with variance $\sigma^2$ each time its sampled. The power spectrum of the signal and its derivatives is,
\begin{equation}
S_s(f) = \frac{A^2}{2} (2\pi f_0)^{2k} T \delta_f^{f_0}
\end{equation}
where $k$ is the number of derivatives and $T$ is length of the time series. The Fourier frequency is $f = \frac{1}{T}$. The noise has the following spectrum,
\begin{equation}
S_n(f) = \sigma^2 \frac{T}{N} (2\pi f)^{2k} 
\end{equation}
where $N$ is the number of samples taken.

The expected total variance can be shown to be,
\begin{equation}
\sum S_n(f) df = \frac{\pi^2}{3} \frac{\sigma^2}{dt^2} \left(1 + \frac{3}{N} + \frac{2}{N^2} \right)
\end{equation}
Compare this to $\frac{2 \sigma^2}{dt^2}$ for finite differencing.

The signal to noise ratio is thus,
\begin{equation}
\frac{S_s(f)}{S_n(f)} = \frac{A^2 N}{2 \sigma^2}  \left( \frac{f_0}{f} \right)^{2k} \delta_f^{f_0}
\end{equation}
which is zero everywhere except at $f_0$. Interestingly the ratio is proportional to the number of samples taken, but is independent of the derivatives. This is not a good example, because in practice a physical process would have a broad band spectrum such that the spectral density wouldn't keep increasing as we zoom in.

Okay, I also did this with the square wave with spectrum,
\begin{equation}
S(f) = \left(\frac{4 A}{n \pi} \right)^2 \frac{T}{2} \delta_f^{(2n-1)f_0}
\end{equation}
With this we know *exactly* the number of degrees of freedom when the signal to noise ratio is large. And the obvious conclusion is that the number of degrees of freedom is just the ratio of the Nyquist to the fundamental square wave frequency. Which, obviously just tells you how much you're sampling it. If the signal drops below the noise, we end up with $N$ degrees of freedom (or very nearly).

Okay, current hypothesis is that we just need to take the ratio of the Nyquist frequency to the frequency where the noise equals the signal.

\section{Back to $\Gamma$}

My $\Gamma$ function isn't horrible and actually works a little bit. So maybe it just needs to be refined. What we're maybe really asking is how often we're going to be able to distinguish noise from signal. So, if the noise looks like a gaussian with $\sigma^2$ variance, and the signal looks like a gaussian with $(u_\textrm{rms} \Delta t)^2$ variance, then we have an interesting statement about probabilities.

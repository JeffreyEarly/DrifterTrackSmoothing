...
For $\nu=3$ this becomes,
\begin{equation}
p_l\left(x|\sigma^2\right) = \frac{2}{\pi \sigma \sqrt{3} } \left( 1 + \frac{x^2}{3 \sigma^2} \right)^{-2}
\end{equation}
...
\end{equation}
where $\rho = \frac{1}{2} \left( \frac{x_i - x(t_i,\mathbf{a})}{\sigma_i} \right)^2$ where $(t_i,x_i)$ is the data and $x(t,\mathbf{a})$ is the model with parameters $\mathbf{a}$. We then try to find the model parameters which maximize the likelihood, or equivalently we try to minimize,
\begin{equation}
\sum_{i=1}^N \rho \left( x_i - x(t_i,\mathbf{a}) \right).
\end{equation}
with respect to $\mathbf{a}$.

Continuing to (mostly) follow Press, et al., now let's define $z_i \equiv  x_i - x(t_i,\mathbf{a})$ and  $\psi \equiv \frac{d \rho}{dz}$. This means that our minimization condition becomes,
\begin{equation}
0 = \sum_{i=1}^N \psi(z_i) \frac{\partial x(t_i,\mathbf{a})}{\partial \mathbf{a}}.
\end{equation}
The Gaussian distribution looks like $\rho(z) = \frac{1}{2} \frac{z^2}{\sigma^2}$ and therefore $\psi(z)=\frac{z}{\sigma^2}$.

Using a Gaussian distribution is equivalent to solving the least-squares problem,
\begin{equation}
0 = \sum_{i=1}^N \frac{z_i}{\sigma_i^2} \frac{\partial x(t_i,\mathbf{a})}{\partial \mathbf{a}},
\end{equation}
...
I have a series of errors $\epsilon_i = y_i - y(x_i)$ that are from some distribution, but I only now how to solve my linear problem for a Gaussian distribution. So I actually have $\epsilon_i/\sigma_i$ looking like a Gaussian distribution to start with. How can I re-weight the values of $\sigma_i$ to make that distribution look like my desired distribution? Quite simply, I want $\epsilon_i/\sigma_i = w(z)$, so $\sigma_i = \epsilon_i/w(z)$.

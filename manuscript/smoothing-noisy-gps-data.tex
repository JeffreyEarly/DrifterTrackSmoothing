...
In the 50 years since \citet{reinsch1967-nm} was published, a number of different methods have been proposed for choosing the optimal value of $\lambda_m$. Both \citet{reinsch1967-nm} and \citet{teanby2007-mg} argue that the tension should be adjusted so that the sample variance is roughly the expected variance, $\hat{\sigma}^2 \sim \sigma^2$, but we will argue that this is only true in very special circumstances. The method of cross-validation \citep{wahba1978-jrss-b,craven1979-nm} is essentially a form of bootstrapping, where the optimal parameter is determined by minimizing the error with data points not included in the fit.

...
Algorithm. We choose $\nu$ and $\sigma_t$ by optimizing the KS test against the motionless GPS data. We then adjust the tension, by optimizing the KS test with the real data, but only within the 99.99\% region. We then \emph{discard} the data points that were outside of the 99.99\% region and re-optimize for the final time, but this time across all values of the cdf. We discard the outliers to reduce the degrees of freedom given to the spline. Given the way we've chosen knots, this means there's a region essentially unsupported by data points. Alternatively, one could just remove some knot points.

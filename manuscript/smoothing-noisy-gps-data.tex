...
\begin{enumerate}
\item does the assumed error PDF match the resulting PDF?
\item does the error correlation structure match our assumptions?
\item does the assumed acceleration PDF match the resulting PDF?
\end{enumerate}
...
When fitting using the Student t-distribution we look for two things, 1) does the autocorrelation sequence look flat and 2) does the acceleration PDF match expectations. We find that if we go to $\sigma=4$, then the matching acceleration PDF (with the same variance) no longer appears to be Gaussian. We want to have the minimum restriction possible on the positions, so setting $\sigma=20$ isn't ideal because we think GPS errors should be smaller than that.

So use the KS statistic to determine when the acceleration PDF looks Gaussian, assuming that we're matching the variance of the distributions.

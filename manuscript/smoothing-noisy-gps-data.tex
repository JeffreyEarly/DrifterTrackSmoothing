...
If we adjust the final tension parameter to so that the resulting error distribution matches the error distribution of the motionless GPS, this will result in an overly smoothed signal. The reason for this is that expected error distribution will depend on the speed of the GPS device.

Show two plots?

The final tension parameter is chosen to preserve the total energy of the signal.

$\epsilon_i \equiv x_i - x(t_i)$
